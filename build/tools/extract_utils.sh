...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$PD_ROOT"/vendor/pixeldust/build/tools/smali/baksmali.jar
        export SMALIJAR="$PD_ROOT"/vendor/pixeldust/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$PD_ROOT"/vendor/pixeldust/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$PD_ROOT"/vendor/pixeldust/build/tools/"$HOST"/compact_dex_converter
    fi
