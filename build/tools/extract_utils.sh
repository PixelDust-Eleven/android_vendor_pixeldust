...
function setup_vendor() {
    ...
    export PD_ROOT="$3"
    if [ ! -d "$PD_ROOT" ]; then
        echo "\$PD_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$PD_ROOT/$OUTDIR" ]; then
        mkdir -p "$PD_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$PD_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$PD_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$PD_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$PD_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
    ...
    if [ -z "$PATCHELF" ]; then
        export PATCHELF="$PD_ROOT"/prebuilts/tools-custom/${HOST}-x86/bin/patchelf
    fi
}
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$PD_ROOT"/prebuilts/tools-custom/common/smali/baksmali.jar
        export SMALIJAR="$PD_ROOT"/prebuilts/tools-custom/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$PD_ROOT"/prebuilts/tools-custom/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$PD_ROOT"/prebuilts/tools-custom/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if grep "classes.dex" "$PD_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$PD_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$PD_ROOT"/vendor/pixeldust/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$PD_ROOT"/"$OUTDIR"/radio
